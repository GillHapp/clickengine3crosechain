...
contract GameMarketplace is ERC1155, ERC1155Holder {
    ...
    mapping(string => mapping(uint256 => address))
        public ownersByAccessIdAndTokenId;
    ...
    function publishNFT(
        ...
        require(
            remainingNFTs[_accessId] == 0,
            'NFTs for this access ID already exist'
        );
    ...
    function setURIForAccessIdAndTokenId(
        string memory _accessId,
        string memory _tokenURI,
    ...
    function buyNFT(string memory _accessId) external payable {
        require(
            remainingNFTs[_accessId] > 0,
            'No NFTs remaining for this access ID'
        );
        require(
            msg.value >= nftsByAccessId[_accessId].sellingPrice,
            'Insufficient funds'
        );
    ...
    function updateOwnerByAccessIdAndTokenId(
        string memory _accessId,
        uint256 _tokenId,
        address _newOwner
    ) internal {
    ...
    function fetchOwnerByAccessIdAndTokenId(
        string memory _accessId,
        uint256 _tokenId
